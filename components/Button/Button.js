...
const Button = ({ children, variant = "green", to = "#" }) => {
  const color = {
  ...
  return (
    <Link href={to}>
      <a
        className={`${styles.container} font-bold inline-block text-center py-4 px-12 h-14 max-h-14 h-auto rounded-md w-full sm:w-auto ${color[variant]}`}
      >
        {children}
      </a>
    </Link>
