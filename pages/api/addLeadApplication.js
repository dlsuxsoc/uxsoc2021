...
const addLeadApplication = async (req, res) => {
  try {
    const data = { ...req.body };
    // Check Existing
    const result = await notion.databases.query({
      database_id: process.env.NOTION_LEAD_APPLICATION,
    });
          ...
          date: {
            start: new Date(
              `${data.birthdate.year}-${String(data.birthdate.month).padStart(
                2,
                "0"
              )}-${String(data.birthdate.date).padStart(
                2,
                "0"
              )}T00:00:00.000+00:00`
            )
            ...
            text: [
              {
                type: "text",
                text: {
                  content: "2. Why are you interested in becoming a lead?",
                },
              },
            ],
          },
...
export default addLeadApplication;
